...
var INTROSCREEN = {
	/* BEGIN :: AUTO-INIT VALUES */
		IntroText : "BRIX",
		CtxFont   : "120px Futura, Helvetica, sans-serif",
		MaxJitter : 15,
		OffsetX   : 20,
		OffsetY   : 10,
		JitterDel : 50,
	...
	fInitialise : function (fCallback) {
		...
		this.OffsetY = (CHeight - this.Metrics.height - 200)/2;

		ctxBoard.font = "20px Comic Sans MS";
		ctxBoard.fillStyle = "#fff";
		ctxBoard.fillText("-: BRIX Version 1.01b [08-07-2011] :-", this.OffsetX, this.OffsetY + this.Metrics.height + 150);
		ctxBoard.fillText("Designed & created by SASWAT PADHI.", this.OffsetX-16, this.OffsetY + this.Metrics.height + 180);

		ctxBoard.font = "24px Times New Roman";
		ctxBoard.fillText("LOADING", 64, CHeight - 64);
		ctxBoard.fillStyle = "#111";
		ctxBoard.strokeStyle = "#aaa";
		ctxBoard.fillRect(186,CHeight - 84,640,24);
		ctxBoard.strokeRect(186,CHeight - 84,640,24);
		this.ShowBar = 1;

		var i;
		ctxBoard.font = this.CtxFont;
		this.GradientL = ctxBoard.createLinearGradient(186,CHeight - 84,640,24);
		for(i = 0; i <= 20; i++) {
			if(i % 2)	this.GradientL.addColorStop(i/20, "rgb(0, 0, 0)");
			else		this.GradientL.addColorStop(i/20, "rgb(0, 255, 255)");
		}
	...
	fUpdScreen : function () {
		...
		if(INTROSCREEN.ShowBar) {
			...
			ctxBoard.fillRect(186,CHeight - 84,(640*Loaded)/LoadMax,24);
		}
	...
	fHideLoadShowPlay : function () {
		...
		ctxBoard.fillRect(64, CHeight - 90, 780, 40);
		ctxBoard.restore();
		if(INTROSCREEN.Transpar)
			...
			ctxBoard.strokeText("~ PLAY ~", INTROSCREEN.OffsetX + 56, CHeight - 48);
			ctxBoard.fillText("~ PLAY ~", INTROSCREEN.OffsetX + 56, CHeight - 48);
			ctxBoard.restore();
	...
	fHandleMouseClick : function (e) {
		...
		var Kx = e.pageX - Z[0];
		var Ky = e.pageY - Z[1];
		if(Kx > INTROSCREEN.OffsetX + 56 && Ky > CHeight - 100 && Kx < INTROSCREEN.OffsetX + 256 && Ky < CHeight - 24) {
...
var BRIXGAME = {
	...
	fResetPaddle : function () {
		...
		this.arrBalls[0].oX = this.arrBalls[0].X;
		this.arrBalls[0].oY = this.arrBalls[0].Y;
	...
	fStartBRIXGame : function () {
		...
		this.BrickProb = MinBrickProb -(this.NowLevel*BrickProbRate);
		this.PointsNow = 0;
		this.PointsStor = 0;
		this.NowLevel = 1;
	...
	fPaintDBallz : function () {
		this.arrBalls.forEach(
			function (val, ind, arr) {
				if(val.V) {
					ctxBoard.drawImage(BacBallz, Math.floor(val.oX), Math.floor(val.oY), BallSize, BallSize);
	...
	fGenerSideB : function () {
		...
		ctxBoard.font = "16px Futura, Helvetica";
		ctxBoard.fillText("Ver. 1.01b", this.BAreaX + 112, 86);

		ctxBoard.font = "32px Comic Sans MS";
		ctxBoard.fillText("~ SCORE ~", this.BAreaX + 6, 176);

		ctxBoard.font = "32px Comic Sans MS";
		ctxBoard.fillText("~ LIVES ~", this.BAreaX + 12, 306);

		ctxBoard.fillStyle = "rgb(255,180,180)";
		ctxBoard.font = "36px Comic Sans MS";
		var k = 0;
		var StrX = "";
		for(k = 0; k < Math.floor(this.LivesLeft/4); ++k) {
			StrX = "&#x2764; &#x2764; &#x2764; &#x2764;"
			DOMCanvas.innerHTML = StrX;
			ctxBoard.fillText(DOMCanvas.innerHTML, this.BAreaX + 8, 346+(k*30));
		}
		var j = k;
		StrX = "";
		for(k = 0; k < (this.LivesLeft%4); ++k)
			StrX += "&#x2764; ";
		DOMCanvas.innerHTML = StrX;
		ctxBoard.fillText(DOMCanvas.innerHTML, this.BAreaX + 8, 346+(j*30));
		j += Math.min(k,1);

		ctxBoard.fillStyle = "rgb(255,255,200)";
		ctxBoard.font = "32px Comic Sans MS";
		ctxBoard.fillText("~ LEVEL ~", this.BAreaX + 6, 376+(j*30));
		ctxBoard.font = "48px Comic Sans MS";
		ctxBoard.fillText(this.NowLevel.toString(), this.BAreaX + 64, 436+(j*30));
	},
	...
	fGameOver : function () {
		...
		ctxBoard.font = "48px Comic Sans MS";
		ctxBoard.fillText("GAME OVER :(", 236, CHeight - 172);
	},
	...
	fLevelClear : function () {
		...
		ctxBoard.fillText("~ LEVEL " + this.NowLevel.toString() + " CLEAR ! ~", 236, CHeight - 172);
		ctxBoard.fillText("LEVEL BONUS : " + (50*this.NowLevel).toString() + " POINTS", 186, CHeight - 132);
		ctxBoard.fillText("<CLICK> TO CONTINUE ...", 206, CHeight - 92);
		this.PointsNow += this.PointsStor + (50*this.NowLevel);
		this.PointsStor = 0;
		this.fPaintSideBar();
		++this.NowLevel;
		DOMCanvas.removeEventListener("click",this.fDiscardClick,false);
	...
	fCollideBallBrick : function (BALL, BRICK, BrickX, BrickY) {
		...
		if (BaL <= BrR && BaR >= BrL && BaT <= BrB && BaB >= BrT) {
			...
			if(FlipX)	BALL.Vx = 0.0 - BALL.Vx;
			if(FlipY)	BALL.Vy = 0.0 - BALL.Vy;
	...
	fSpeedUp : function () {
		this.arrBalls.forEach(
			function (val, ind, arr) {
				if(val.V) {
					val.Vx *= BRIXGAME.SpUpMult;
					val.Vy *= BRIXGAME.SpUpMult;
	...
	fSlowDown : function () {
		this.arrBalls.forEach(
			function (val, ind, arr) {
				if(val.V) {
					val.Vx *= BRIXGAME.SpDnMult;
					val.Vy *= BRIXGAME.SpDnMult;
	...
	fPaintPowers : function () {
		if(this.LastMsgInt >= 12) {
			ctxBoard.font = "22px Times New Roman";
			ctxBoard.fillStyle = "rgb(" + this.LastMsgInt + "," + this.LastMsgInt + "," + this.LastMsgInt + ")";
			ctxBoard.fillText(this.LastMsg, BRIXGAME.BAreaX+16, CHeight - 32);
			this.LastMsgInt -= 2;
		}
			...
			function (val, ind, arr) {
				if(val.V) {
					...
					if(val.Y >= BRIXGAME.PaddleY - PowerSize && val.Y <= BRIXGAME.PaddleY + BRIXGAME.PaddleH && val.X >= BRIXGAME.PaddleX - PowerSize && val.X < BRIXGAME.PaddleX + BRIXGAME.PaddleW)
					{
						...
						ctxBoard.fillStyle = "rgb(12,12,12)";
						ctxBoard.fillRect(BRIXGAME.BAreaX+16, CHeight - 55, 128, 23);

						BRIXGAME.PointsStor += val.Pnt;
						BRIXGAME.LastMsg = val.P.Msg;
						BRIXGAME.LastMsgInt = 254;
					}
				}
			}
		);
			...
			function (val, ind, arr) {
				if(val.V) {
					...
					if(val.Vy > 0 && val.X >= BRIXGAME.PaddleX - BallSize && val.X < BRIXGAME.PaddleX + BRIXGAME.PaddleW && val.Y >= BRIXGAME.PaddleY - BallSize && val.Y <= BRIXGAME.PaddleY) {
						var theta = 75.0*(((BRIXGAME.PaddleX + (BRIXGAME.PaddleW/2)) - val.X)/(BRIXGAME.PaddleW/2));
						theta += 90.0;
						theta /= 180.0;
						theta *= 3.1415;
	...
	fPaintSideBar : function () {
		...
		ctxBoard.fillStyle = "rgb(12,12,12)";
		ctxBoard.fillRect(this.BAreaX+8, 190, 170, 80);

		ctxBoard.font = "70px Comic Sans MS";
		ctxBoard.lineWidth = 4;
		ctxBoard.strokeStyle = "rgba(" + (Math.floor(Math.random()*(255 - MinRED))+MinRED).toString() + "," + (Math.floor(Math.random()*(255 - MinGRN))+MinGRN).toString() + "," + (Math.floor(Math.random()*(255 - MinBLU))+MinBLU).toString() + ",0.8)";
		ctxBoard.strokeText("BRIX", this.BAreaX + 2, 64);
		ctxBoard.font = "68px Comic Sans MS";
		ctxBoard.fillStyle = "rgb(255,255,200)";
		ctxBoard.fillText("BRIX", this.BAreaX + 4, 64);
		ctxBoard.font = "48px Comic Sans MS";
		ctxBoard.fillText(this.PointsNow.toString(), this.BAreaX + 12, 236);

		ctxBoard.fillStyle = "green";
		ctxBoard.fillRect(this.BAreaX,CHeight - 8,Math.floor((1 - (this.BrickCount/this.BCBackUp))*(CWidth-this.BAreaX)),8)
	},

	fRunLevelDynamics : function () {
		if(BRIXGAME.PointsStor) {
			BRIXGAME.PointsStor--;
			BRIXGAME.PointsNow++;
		}
		...
		if(BRIXGAME.PauseGame) {
			...
			ctxBoard.fillStyle = "yellow";
			ctxBoard.font = "28px Comic Sans MS";
			ctxBoard.fillText("<CLICK> TO START :) ", 256, CHeight - 132);
		}
		else {
			BRIXGAME.Pausable = 1;
	...
	fPauseGame : function () {
		...
		ctxBoard.fillStyle = "yellow";
		ctxBoard.font = "28px Comic Sans MS";
		ctxBoard.fillText("PAUSED : <CLICK> TO RESUME", 160, CHeight - 132);
			...
			BRIXGAME.arrBalls.forEach(
				function (val, ind, arr) {
					if(val.V) {
						...
						val.oX += FinalX - BRIXGAME.PaddleX;
					}
				}
			);
